...
lux start --nodes 3                    # grow into a signed 3-node local cluster
lux cluster status                     # inspect topology and node convergence
lux cluster resize 2                   # safely redistribute and remove a node
lux cluster consolidate                # restore the standalone single-node fast path
...
| `LUX_CLUSTER_CONFIG` | (disabled) | Path to this node's signed Cluster configuration. Normally generated by `lux start --nodes N` or the Lux Cloud controller |
