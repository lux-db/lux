...
function collectionCommands(count: number): Iterable<string[]> {
	return (function* () {
		for (let index = 0; index < count; index++) {
			...
			yield [
				"HSET",
				`benchmark:hash:${suffix}`,
				"field",
				`value-${index}`,
			];
			yield [
				"LPUSH",
				`benchmark:list:${suffix}`,
				...Array.from({ length: 10 }, (_, item) => `item-${item}`),
			];
			yield [
				"SADD",
				`benchmark:set:${suffix}`,
				...Array.from({ length: 10 }, (_, item) => `member-${item}`),
			];
			yield [
				"ZADD",
				`benchmark:zset:${suffix}`,
...
export function compatibilityWorkloads(
	...
	const pipelines = [
		...new Set(
			quick
				? [1, settings.pipeline]
				: [1, settings.pipeline, 64, 128, 256, 512],
		),
	...
	const collectionCases: Array<[string, string, string[]]> = [
		[
			"hash_write",
			"HSET",
			["HSET", "benchmark:hash:__rand_int__", "field", "updated"],
		],
		["hash_read", "HGET", ["HGET", "benchmark:hash:__rand_int__", "field"]],
		[
			"list_write",
			"LPUSH",
			["LPUSH", "benchmark:list:__rand_int__", "new-item"],
		],
		[
			"list_range",
			"LRANGE",
			["LRANGE", "benchmark:list:__rand_int__", "0", "9"],
		],
		[
			"set_write",
			"SADD",
			["SADD", "benchmark:set:__rand_int__", "new-member"],
		],
		[
			"set_membership",
			"SISMEMBER",
			["SISMEMBER", "benchmark:set:__rand_int__", "member-5"],
		],
		[
			"sorted_set_write",
			"ZADD",
			["ZADD", "benchmark:zset:__rand_int__", "11", "new-member"],
		],
		[
			"sorted_set_range",
			"ZRANGE",
			["ZRANGE", "benchmark:zset:__rand_int__", "0", "9"],
		],
	];
	...
	const geo = [
		...
		[
			"GEODIST",
			["GEODIST", "benchmark:geo", "place:100", "place:500", "km"],
		],
		[
			"GEOSEARCH 500km",
			[
			...
			connections.map(async (connection) => {
				while (true) {
					...
					const key = benchmarkKey(
						"benchmark:string:128",
						(index * 7_919) % keyspace,
					);
					...
					if (read && typeof value !== "string")
						throw new Error(
							`${subject.kind} mixed read returned ${JSON.stringify(value)}`,
						);
					if (!read && value !== "OK")
						throw new Error(
							`${subject.kind} mixed write returned ${JSON.stringify(value)}`,
						);
				}
			}),
		);
	...
	for (const readPercent of [90, 50]) {
		...
		for (
			...
			for (const subject of order) {
				...
				progress(
					`  ${subject.kind} ${readPercent}/${100 - readPercent} run ${repetition + 1}: ${sample.rps.toFixed(0)} ops/s, p99 ${sample.p99_latency_ms.toFixed(3)} ms`,
				);
			}
		}
		for (const subject of subjects) {
...
export async function measureCompatibility(
	...
	for (const subject of subjects) {
		...
		await loadCommands(
			...
			BENCHMARK_PASSWORD,
			collectionCommands(
				Math.min(settings.keyspace, quick ? 1_000 : 10_000),
			),
		);
	}
	...
	for (const subject of subjects) {
		...
		try {
			const value = await connection.request([
				"GET",
				benchmarkKey("benchmark:string:128", 0),
			]);
			if (value !== "x".repeat(128))
				throw new Error(
					`${subject.kind} string dataset changed unexpectedly`,
				);
			const position = await connection.request([
				"GEOPOS",
				"benchmark:geo",
				"place:500",
			]);
			if (!Array.isArray(position) || position.length !== 1)
				throw new Error(
					`${subject.kind} geospatial dataset is incomplete`,
				);
		} finally {
			connection.close();
		}
