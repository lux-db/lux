...
describe("benchmark catalog", () => {
	test("standard compatibility catalog is valid and retains the full pipeline curve", () => {
		...
		expect(
			workloads.some((workload) => workload.id === "pipeline_set_512"),
		).toBeTrue();
		expect(
			workloads.some(
				(workload) => workload.id === "geo_geosearch_5000km_512",
			),
		).toBeTrue();
		expect(
			workloads.some((workload) => workload.group === "Data structures"),
		).toBeTrue();
	});
	...
	test("pipeline override is represented in the quick curve", () => {
		...
		expect(
			workloads.some((workload) => workload.id === "pipeline_get_64"),
		).toBeTrue();
	});
});
