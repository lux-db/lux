...
export function validateConfiguration(workloads: Workload[]): void {
  ...
  for (const workload of workloads) {
    ...
    if (ids.has(workload.id)) throw new Error(`duplicate workload id: ${workload.id}`);
    if (workload.command.length === 0) throw new Error(`${workload.id} has no command`);
    ids.add(workload.id);
  }
  ...
  for (const [name, scale] of Object.entries(SCALES)) {
    for (const [field, value] of Object.entries(scale)) {
      ...
      if (values.some((entry) => !Number.isSafeInteger(entry) || entry <= 0))
        throw new Error(`${name}.${field} must contain positive integers`);
    }
  }
}
