import { BENCHMARK_PASSWORD, type BenchmarkScale, type BenchmarkSettings } from "./config";
...
function datasetValue(index: number, bytes: number): string {
  const prefix = `${index}:`;
  if (prefix.length > bytes) throw new Error(`key index ${index} exceeds payload width ${bytes}`);
...
function datasetCommands(count: number, bytes: number): Iterable<string[]> {
  return (function* () {
    for (let index = 0; index < count; index++)
      yield ["SET", `benchmark:recovery:${index}`, datasetValue(index, bytes)];
  })();
}
...
async function currentMemory(subject: Subject): Promise<number> {
  ...
  const usage = String(stats.MemUsage ?? "").split("/")[0].trim();
...
async function waitHostReady(subject: Subject): Promise<void> {
  ...
  throw new Error(`${subject.kind} host port did not become ready after restart`);
}

async function readTraffic(subject: Subject, requests: number, datasetKeys: number): Promise<Sample> {
  ...
  try {
    await Promise.all(connections.map(async (connection) => {
      while (true) {
        ...
        const value = await connection.request(["GET", `benchmark:recovery:${key}`]);
        latencies.push(performance.now() - requestStarted);
        if (typeof value !== "string" || !value.startsWith(`${key}:`))
          throw new Error(`${subject.kind} returned an incorrect value during snapshot`);
      }
    }));
  } finally {
...
async function snapshot(subject: Subject): Promise<{ duration: number; peak: number }> {
  const connection = await RespConnection.connect(subject.resp_port, BENCHMARK_PASSWORD);
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const info = parseInfo(String(await connection.request(["INFO", "persistence"])));
      if (info.rdb_bgsave_in_progress === "0") {
        if (info.rdb_last_bgsave_status !== "ok")
          throw new Error(`${subject.kind} snapshot failed: ${JSON.stringify(info)}`);
        return { duration: performance.now() - started, peak };
      }
      await Bun.sleep(10);
    }
    throw new Error(`${subject.kind} snapshot did not complete within 120 seconds`);
  } finally {
    connection.close();
  }
}
...
async function verifyDataset(
  ...
  try {
    ...
    if (Number(size) !== count + postSnapshotKeys)
      throw new Error(`${subject.kind} recovered ${size} keys; expected ${count + postSnapshotKeys}`);
    for (let sample = 0; sample < samples; sample++) {
      const index = Math.floor((sample * count) / samples);
      const value = await connection.request(["GET", `benchmark:recovery:${index}`]);
      if (value !== datasetValue(index, payloadBytes))
        throw new Error(`${subject.kind} recovered an incorrect value for key ${index}`);
    }
    for (let index = 0; index < postSnapshotKeys; index++) {
      const value = await connection.request(["GET", `benchmark:post-snapshot:${index}`]);
      if (value !== `post-snapshot-${index}`)
        throw new Error(`${subject.kind} did not recover post-snapshot key ${index}`);
    }
...
export async function measureLifecycle(
  ...
  for (const keyCount of scale.recoveryKeys) {
    const cases = [
      { kind: "lux" as const, mode: "durable" as const, storage: "memory" as const },
      { kind: "redis" as const, mode: "durable" as const, storage: "redis_aof" as const },
      { kind: "lux" as const, mode: "tiered" as const, storage: "tiered" as const },
    ];
    for (const testCase of cases) {
      const { kind, mode, storage } = testCase;
      progress(`Lifecycle: ${kind}/${storage}, ${keyCount} durable 1 KiB values...`);
      ...
      try {
        ...
        await loadCommands(
          ...
          (function* () {
            for (let index = 0; index < postSnapshotKeys; index++)
              yield ["SET", `benchmark:post-snapshot:${index}`, `post-snapshot-${index}`];
          })(),
        );
        await Bun.sleep(1_100);
        ...
        await command(["docker", "volume", "rm", `${subject.name}-data`], {
          allowFailure: true,
          quiet: true,
        });
      }
