	...
	return (function* () {
		for (let index = 0; index < vectors.length; index++) {
			yield [
				...
				String(vectors[index].length),
				...vectors[index].map((value) => value.toFixed(7)),
				"META",
				JSON.stringify({ cohort: index % 2 === 0 ? "even" : "odd" }),
			];
		}
	})();
}
...
function resultKeys(value: unknown): string[] {
	if (!Array.isArray(value))
		throw new Error(`vector search returned ${JSON.stringify(value)}`);
	return value.map((entry) => {
		if (!Array.isArray(entry) || typeof entry[0] !== "string")
			throw new Error(
				`vector search returned malformed entry ${JSON.stringify(entry)}`,
			);
		return entry[0];
	});
}
...
async function concurrentRespSample(
	...
	const connections = await Promise.all(
		Array.from({ length: concurrency }, () =>
			RespConnection.connect(port, BENCHMARK_PASSWORD),
		),
	);
	let next = 0;
...
export async function measureVectors(
	...
	progress(
		`Preparing ${scale.vectorCount} ${scale.vectorDimensions}-dimensional vectors...`,
	);
	await redisCli(subject, network, clientImage, "FLUSHALL");
	const vectors = Array.from({ length: scale.vectorCount }, (_, index) =>
		seededVector(index, scale.vectorDimensions),
	);
	await loadCommands(
		subject.resp_port,
		BENCHMARK_PASSWORD,
		vectorCommands(vectors),
		128,
	);
	...
	const queryIndexes = Array.from(
		...
		(_, index) =>
			Math.floor((index * scale.vectorCount) / scale.vectorQueries),
	);
	const k = Math.min(10, scale.vectorCount);
	let matches = 0;
	let filteredMatches = 0;
	let possible = 0;
	const verifier = await RespConnection.connect(
		subject.resp_port,
		BENCHMARK_PASSWORD,
	);
	try {
		for (const queryIndex of queryIndexes) {
			...
			const actual = resultKeys(
				await verifier.request([
					"VSEARCH",
					String(scale.vectorDimensions),
					...query.map((value) => value.toFixed(7)),
					"K",
					String(k),
				]),
			);
			...
			if (!actual.includes(`benchmark:vector:${queryIndex}`))
				throw new Error(
					`vector search did not return its exact source vector ${queryIndex}`,
				);
			const cohort = queryIndex % 2 === 0 ? "even" : "odd";
			const filtered = resultKeys(
				await verifier.request([
					"VSEARCH",
					String(scale.vectorDimensions),
					...query.map((value) => value.toFixed(7)),
			...
			const filteredExpected = vectors
				...
				.filter(
					(entry) =>
						(entry.index % 2 === 0 ? "even" : "odd") === cohort,
				)
				.sort((left, right) => right.score - left.score)
				.slice(0, k)
				.map((entry) => entry.key);
			const filteredExpectedSet = new Set(filteredExpected);
			filteredMatches += filtered.filter((key) =>
				filteredExpectedSet.has(key),
			).length;
			if (
				filtered.some(
					(key) =>
						Number(key.split(":").at(-1)) % 2 !== queryIndex % 2,
				)
			)
				throw new Error(
					`filtered vector search returned the wrong cohort for ${queryIndex}`,
				);
		}
	} finally {
		verifier.close();
	...
	await concurrentRespSample(
		...
		(value) => {
			if (resultKeys(value).length !== k)
				throw new Error(
					"vector search returned the wrong result count",
				);
		},
	);
	for (let repetition = 0; repetition < settings.repetitions; repetition++) {
		const sample = await concurrentRespSample(
			...
			(index) => {
				const query =
					vectors[
						queryIndexes[(index + repetition) % queryIndexes.length]
					];
				return [
					"VSEARCH",
					String(query.length),
		...
		samples.push(sample);
		progress(
			`  vector search run ${repetition + 1}: ${sample.rps.toFixed(0)} ops/s, p99 ${sample.p99_latency_ms.toFixed(3)} ms`,
		);
		const filtered = await concurrentRespSample(
			...
			(value, index) => {
				...
				if (
					...
					keys.some(
						(key) =>
							Number(key.split(":").at(-1)) % 2 !==
							expectedParity,
					)
				)
					throw new Error(
						"filtered vector search returned an invalid result set",
					);
			},
		);
		filteredSamples.push(filtered);
		progress(
			`  filtered vector search run ${repetition + 1}: ${filtered.rps.toFixed(0)} ops/s, p99 ${filtered.p99_latency_ms.toFixed(3)} ms`,
		);
	}

	const writeVector = seededVector(
		scale.vectorCount + 1,
		scale.vectorDimensions,
	...
	const writeWorkload: Workload = {
		...
		command: [
			"VSET",
			"benchmark:vector:write:__rand_int__",
			String(scale.vectorDimensions),
			...writeVector,
		],
	...
	measurements.push({
		...
		subject: "lux",
		description:
			"Top-k search across varied queries with result validation",
		dimensions: {
			operation: "search",
			requests,
...
export async function measureTimeSeries(
	...
	progress(
		`Preparing ${total} samples across ${scale.timeSeriesCount} time series...`,
	);
	await redisCli(subject, network, clientImage, "FLUSHALL");
	await loadCommands(
		subject.resp_port,
		BENCHMARK_PASSWORD,
		timeSeriesCommands(scale),
	);
	...
	try {
		const range = await verify.request([
			"TSRANGE",
			`benchmark:ts:${String(0).padStart(12, "0")}`,
			"0",
			String(scale.samplesPerSeries - 1),
		]);
		if (!Array.isArray(range) || range.length !== scale.samplesPerSeries)
			throw new Error(
				`time-series verification returned ${Array.isArray(range) ? range.length : "non-array"} samples`,
			);
	} finally {
		verify.close();
	}
