...
function compatibilityTable(lines: string[], entries: Measurement[]): void {
  ...
  for (const pair of workloads.values()) {
    ...
    if (!lux || !redis) throw new Error(`comparison workload ${pair[0].workload} is incomplete`);
    const ratio = redis.summary.rps === 0 ? "-" : `${(lux.summary.rps / redis.summary.rps).toFixed(2)}x`;
    lines.push(
...
export function markdown(result: ReportResult): string {
  ...
  for (const [group, entries] of groups(result.measurements)) {
    lines.push("", `## ${group}`);
    if (entries[0].class === "compatibility") compatibilityTable(lines, entries);
  ...
  if (result.resource_samples.length) {
    ...
    for (const sample of result.resource_samples) {
      for (const [subject, resource] of Object.entries(sample.subjects))
        lines.push(`| ${sample.stage} | ${subject} | ${resource.MemUsage ?? "unavailable"} | ${sample.description} |`);
    }
  }
  lines.push("", "Run with `--json` to emit raw repetitions and complete configuration.", "");
  return lines.join("\n");
}
