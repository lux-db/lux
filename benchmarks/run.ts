...
async function main(): Promise<void> {
	...
	validateConfiguration(
		compatibilityWorkloads(options.quick, options.settings),
	);
	if (options.validate) {
		console.log(
			`Benchmark harness v${HARNESS_VERSION}: configuration valid`,
		);
		return;
	}
	...
	if (!options.redisImage.includes("@sha256:"))
		console.error(
			"Note: the Redis override is not digest-pinned; the resolved image ID will still be recorded.",
		);
	...
	const remember = (subject: Subject) => {
		...
		if (
			subject.configuration.LUX_DURABILITY &&
			subject.configuration.LUX_DURABILITY !== "ephemeral"
		)
			volumes.add(`${subject.name}-data`);
		if (
			subject.kind === "redis" &&
			subject.configuration.persistence !== "none"
		)
			volumes.add(`${subject.name}-data`);
		return subject;
	};
	...
	try {
		if (!luxImage) {
			...
			await command(
				[
					...
					"build",
					"--file",
					resolve(root, LUX_BUILD.dockerfile),
					"--build-arg",
					`LUX_BUILD_SHA=${commit.stdout.trim()}`,
					"--build-arg",
					"LUX_VERSION=local-benchmark",
					"--label",
					`org.opencontainers.image.revision=${commit.stdout.trim()}`,
					"--label",
					"org.opencontainers.image.version=local-benchmark",
					"--tag",
					luxImage,
					root,
				],
			...
			await command(["docker", "image", "inspect", luxImage], {
				quiet: true,
			});
		}
		...
		if (options.features.has("compatibility")) {
			...
			resourceSamples.push({
				stage: "compatibility",
				description:
					"Resident memory after the disclosed compatibility datasets and measurements",
				subjects: {
					lux: await containerStats(lux),
					redis: await containerStats(redis),
				},
			});
		...
		if (options.features.has("vectors")) {
			...
			const result = await measureVectors(
				lux,
				network,
				options.redisImage,
				settings,
		...
		if (options.features.has("realtime")) {
			...
			for (const fanout of options.quick ? [1, 10] : [1, 10, 100]) {
				...
				measurements.push({
					...
					subject: "lux",
					description:
						"Acknowledged mutation through receipt by every subscriber",
					dimensions: {
			...
			resourceSamples.push({
				stage: "realtime",
				description:
					"Resident memory after fanout measurements; every expected delivery was verified",
				subjects: { lux: await containerStats(lux) },
			});
			await retire(lux);
		}
		...
		if (options.features.has("tiered")) {
			...
			resourceSamples.push({
				stage: "tiered",
				description:
					"Resident memory after verified hot, cold, and mixed reads",
		...
		const result = {
			...
			subjects: subjects.map(
				({ kind, alias, image, startup_ms, configuration }) => ({
					kind,
					alias,
					image,
					startup_ms,
		...
		if (temporaryImage)
			await command(["docker", "image", "rm", temporaryImage], {
				allowFailure: true,
				quiet: true,
			});
	}
