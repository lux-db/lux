...
export function summarize(samples: Sample[]): Summary {
  const values = <K extends keyof Sample>(key: K) => samples.map((sample) => sample[key]);
...
export function sampleFromLatencies(latencies: number[], elapsedMs: number): Sample {
  if (latencies.length === 0) throw new Error("cannot measure an empty workload");
  ...
  return {
    ...
    avg_latency_ms: sorted.reduce((sum, value) => sum + value, 0) / sorted.length,
...
export function parseBenchmarkCsv(output: string): Sample {
  ...
  if (rows[0].some((value, index) => value !== expected[index]))
    throw new Error(`unexpected redis-benchmark columns: ${rows[0].join(",")}`);
