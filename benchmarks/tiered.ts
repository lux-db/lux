import { BENCHMARK_PASSWORD, tieredMemoryLimit, type BenchmarkSettings, type Workload } from "./config";
import { measureCatalog, parseInfo, redisCli, type Measurement } from "./measure";
...
function tieredCommands(prefix: string, count: number, payload: string): Iterable<string[]> {
  return (function* () {
  ...
  try {
    await Promise.all(connections.map(async (connection) => {
      while (true) {
        ...
        const keyIndex = hot ? index % hotCount : (coldOffset + Math.floor(index / 10)) % coldCount;
        const expected = hot ? "hot" : "cold";
        const requestStarted = performance.now();
        const value = await connection.request([
          "GET", `benchmark:tiered:${expected}:${padded(keyIndex)}`,
        ]);
        latencies.push(performance.now() - requestStarted);
        if (typeof value !== "string" || !value.startsWith(expected))
          throw new Error(`tiered ${expected} read returned ${JSON.stringify(value)}`);
      }
    }));
  } finally {
  ...
  try {
    await Promise.all(connections.map(async (connection) => {
      while (true) {
        ...
        const requestStarted = performance.now();
        const value = await connection.request([
          "GET", `benchmark:tiered:cold:${padded(index)}`,
        ]);
        latencies.push(performance.now() - requestStarted);
        if (typeof value !== "string" || !value.startsWith("cold"))
          throw new Error(`tiered cold read returned ${JSON.stringify(value)}`);
      }
    }));
  } finally {
    for (const connection of connections) connection.close();
...
export async function measureTiered(
  ...
  const coldCount = settings.warmupRequests
    + settings.requests * settings.repetitions
    + Math.ceil(settings.requests * settings.repetitions * 0.1)
    + 1_000;
  const hotCount = quick ? 500 : 2_000;
  progress(`Preparing ${coldCount} cold and ${hotCount} hot tiered values...`);
  await loadCommands(
    subject.resp_port,
    BENCHMARK_PASSWORD,
    tieredCommands("benchmark:tiered:cold", coldCount, "cold".padEnd(1_024, "x")),
  );
  await loadCommands(
    subject.resp_port,
    BENCHMARK_PASSWORD,
  ...
  const before = parseInfo(await redisCli(subject, network, clientImage, "INFO", "storage"));
  ...
  const workloads: Workload[] = [
    {
      ...
      dimensions: { operation: "hot_read", hot_percent: 100, keys: hotCount },
    },
  ];
  ...
  for (let repetition = 0; repetition < settings.repetitions; repetition++) {
    const sample = await uniqueColdSample(subject, settings.requests, concurrency, coldOffset);
    coldOffset += settings.requests;
    cold.push(sample);
    progress(`  unique cold read run ${repetition + 1}: ${sample.rps.toFixed(0)} ops/s, p99 ${sample.p99_latency_ms.toFixed(3)} ms`);
  }
  measurements.push({
  ...
  const after = parseInfo(await redisCli(subject, network, clientImage, "INFO", "storage"));
  return {
    measurements,
    evidence: [{
      before_reads: before,
      ...
      within_configured_memory: Number(after.used_memory_bytes) <= configured,
    }],
  };
}
