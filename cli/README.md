...
The local engine uses tiered placement with explicit `always_sync` durability,
so successful writes are synchronized before acknowledgement. `lux status`
reports both the storage layout and durability policy so disk placement is
never mistaken for the write acknowledgement guarantee.
