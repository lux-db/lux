...
// WAL-replay recovery. KNOWN BUG (reliability audit): the WAL logs the raw
// `TINSERT` (no id) and replay re-runs `resolve_default`/auto-uuid, so the row's
// primary key silently CHANGES after a crash recovered from WAL. Same root cause
// as the TTL-reset-on-replay behavior. Ignored until the WAL logs the resolved
// command/row instead of the raw input.
#[test]
#[ignore = "known bug: WAL replay regenerates auto-uuid/now() values; fix = log resolved command"]
